return(function(Installer)
    ...
    local Quantity = ItemReplicationService._UserCache[LocalPlayer.UserId]:GetItems("Quantity")
    local Mastery = ItemReplicationService._UserCache[LocalPlayer.UserId]:GetItems("Mastery")
    ...
    AddModule("Inventory", function()
        ...
        function Cache:Search(High)
            ...
            for _, v in next, Quantity do
                local Result = ItemConfig.match(v.ItemId)
                local Item = Result and Result:isOk() and Result:unwrap()

                if not Item or Item.Index.IdType ~= "PhysicalMoveset" then continue end

                local Name = Item.Index.DebugLabel:gsub(" %[.-%]", "")

                for FruitName, Value in List do
                    if Name ~= FruitName then continue end
        ...
        task.spawn(function()
            for _, v in next, Mastery do
                for _, x in next, Quantity do
                    if x.ItemId == v.ItemId then
                        local Result = ItemConfig.match(x.ItemId)

                        if Result and Result:isOk() then
                            local Item = Result:unwrap()
                            local Name = Item.Index.DebugLabel:gsub(" %[.-%]", "")
